  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    ...
    | (lexer::TokenKind::VAR(_), _) => CodeType::Tok1,
    (lexer::TokenKind::LETNONREC, _) | (lexer::TokenKind::LETREC, _) => CodeType::Tok2,
  ...
  let main = match code_type? {
    ...
    CodeType::Tok2 => _parse_fn_nxtoplevel(tokens)?,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::LETNONREC, _) => CodeType::Tok1,
    (lexer::TokenKind::LETREC, _) => CodeType::Tok2,
  ...
  let main = match code_type? {
    ...
    CodeType::Tok2 => {
      let opn = _parse_token_Tok_LETREC(tokens)?;
      let recdec = _parse_fn_nxnonrecdec(tokens)?;
      let subseq = _parse_fn_nxtopsubseq(tokens)?;
      let (name, utast) = recdec;
      let (_, opnrng) = opn;
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::VAR(_), _) => CodeType::Tok1,
    (lexer::TokenKind::LPAREN, _) => CodeType::Tok2,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::BINOP_AMP(_), _) => CodeType::Tok1,
    (lexer::TokenKind::BINOP_BAR(_), _) => CodeType::Tok2,
    (lexer::TokenKind::BINOP_DIVIDES(_), _) => CodeType::Tok3,
    (lexer::TokenKind::BINOP_EQ(_), _) => CodeType::Tok4,
    (lexer::TokenKind::BINOP_GT(_), _) => CodeType::Tok5,
    (lexer::TokenKind::BINOP_HAT(_), _) => CodeType::Tok6,
    (lexer::TokenKind::BINOP_LT(_), _) => CodeType::Tok7,
    (lexer::TokenKind::BINOP_MINUS(_), _) => CodeType::Tok8,
    (lexer::TokenKind::BINOP_PLUS(_), _) => CodeType::Tok9,
    (lexer::TokenKind::BINOP_TIMES(_), _) => CodeType::Tok10,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::LETNONREC, _) | (lexer::TokenKind::LETREC, _) => CodeType::Tok1,
    (lexer::TokenKind::EOF, _) => CodeType::Tok2,
    (lexer::TokenKind::IN, _) => CodeType::Tok3,

    _ => CodeType::Other,
  });
  let main = match code_type? {
    CodeType::Tok1 => _parse_fn_nxtoplevel(tokens)?,
    CodeType::Tok2 => {
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    ...
    | (lexer::TokenKind::LPAREN, _)
    | (lexer::TokenKind::TRUE, _)
    | (lexer::TokenKind::VAR(_), _) => CodeType::Tok3,
  ...
  let main = match code_type? {
    ...
    CodeType::Tok3 => _parse_fn_nxif(tokens)?,
    _ => return Err(ParseError::UnexpectedToken(tokens.next().unwrap())),
  };
  Ok(main)
}
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::IF, _) => CodeType::Tok1,
    (lexer::TokenKind::FALSE, _)
    | (lexer::TokenKind::FLOATCONST(_), _)
    | (lexer::TokenKind::INTCONST(_), _)
    | (lexer::TokenKind::LAMBDA, _)
    | (lexer::TokenKind::LPAREN, _)
    | (lexer::TokenKind::TRUE, _)
    | (lexer::TokenKind::VAR(_), _) => CodeType::Tok2,
  ...
  let main = match code_type? {
    ...
    CodeType::Tok2 => _parse_fn_lor(tokens)?,
    _ => return Err(ParseError::UnexpectedToken(tokens.next().unwrap())),
  };
  Ok(main)
}
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::BINOP_BAR(_), _) => CodeType::Tok1,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::BINOP_EQ(_), _) => CodeType::Tok1,
    (lexer::TokenKind::BINOP_LT(_), _) => CodeType::Tok2,
    (lexer::TokenKind::BINOP_GT(_), _) => CodeType::Tok3,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::BINOP_HAT(_), _) => CodeType::Tok1,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::BINOP_PLUS(_), _) => CodeType::Tok1,
    (lexer::TokenKind::BINOP_MINUS(_), _) => CodeType::Tok2,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::BINOP_TIMES(_), _) => CodeType::Tok1,
    (lexer::TokenKind::BINOP_DIVIDES(_), _) => CodeType::Tok2,
  ...
  Tokens: Iterator<Item = lexer::Token>,
{
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::INTCONST(_), _) => CodeType::Tok1,
    (lexer::TokenKind::FLOATCONST(_), _) => CodeType::Tok2,
    (lexer::TokenKind::TRUE, _) => CodeType::Tok3,
    (lexer::TokenKind::FALSE, _) => CodeType::Tok4,
    (lexer::TokenKind::LPAREN, _) => CodeType::Tok5,
    (lexer::TokenKind::VAR(_), _) => CodeType::Tok6,
    (lexer::TokenKind::LAMBDA, _) => CodeType::Tok7,
  ...
  let main = match code_type? {
    ...
    CodeType::Tok5 => {
      ...
      let utast_and_rng = _parse_fn_paren_nxlet_or_binop(tokens)?;
...
#[allow(non_snake_case)]
#[allow(unused_parens)]
fn _parse_fn_paren_nxlet_or_binop<Tokens>(
  tokens: &mut Peekable<Tokens>,
) -> Result<(UntypedAST, Range), ParseError>
where
  Tokens: Iterator<Item = lexer::Token>,
{
  enum CodeType {
  ...
  let code_type = tokens.peek().ok_or(ParseError::Eof).map(|tok| match tok {
    (lexer::TokenKind::BINOP_TIMES(_), _) => CodeType::Tok1,
    (lexer::TokenKind::BINOP_DIVIDES(_), _) => CodeType::Tok2,
    (lexer::TokenKind::BINOP_PLUS(_), _) => CodeType::Tok3,
    (lexer::TokenKind::BINOP_MINUS(_), _) => CodeType::Tok4,
    (lexer::TokenKind::BINOP_HAT(_), _) => CodeType::Tok5,
    (lexer::TokenKind::BINOP_AMP(_), _) => CodeType::Tok6,
    (lexer::TokenKind::BINOP_BAR(_), _) => CodeType::Tok7,
    (lexer::TokenKind::BINOP_GT(_), _) => CodeType::Tok8,
    (lexer::TokenKind::BINOP_LT(_), _) => CodeType::Tok9,
    (lexer::TokenKind::BINOP_EQ(_), _) => CodeType::Tok10,
    (lexer::TokenKind::FALSE, _)
    | (lexer::TokenKind::FLOATCONST(_), _)
    | (lexer::TokenKind::IF, _)
    | (lexer::TokenKind::INTCONST(_), _)
    | (lexer::TokenKind::LAMBDA, _)
    | (lexer::TokenKind::LETNONREC, _)
    | (lexer::TokenKind::LETREC, _)
    | (lexer::TokenKind::LPAREN, _)
    | (lexer::TokenKind::TRUE, _)
    | (lexer::TokenKind::VAR(_), _) => CodeType::Tok11,
...
#[allow(non_snake_case)]
#[allow(unused_parens)]
fn _parse_token_Tok_LETREC<Tokens>(
  tokens: &mut Peekable<Tokens>,
) -> Result<lexer::Token, ParseError>
where
  Tokens: Iterator<Item = lexer::Token>,
{
  tokens
    ...
    .ok_or(ParseError::Eof)
    .and_then(|tok| match tok.clone() {
      (lexer::TokenKind::LETREC, _) => Ok(tok),
      _ => Err(ParseError::UnexpectedToken(tok)),
    })
}
