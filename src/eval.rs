...
pub enum Exval {
  ...
  RecProcV(String, String, types::UntypedAST, environment::Env<Dnval>),
...
pub fn eval_exp(env: environment::Env<Dnval>, ast: types::UntypedAST) -> Dnval {
  ...
  match astmain {
    ...
    UntypedASTMain::ContentOf(_, x) => match environment::lookup(x.clone(), env) {
      Some(e) => e,                            //ProcVを返す
    ...
    UntypedASTMain::BinApply(op, exp1, exp2) => {
      ...
      match environment::lookup(op_string.clone(), env) {
        Some(e) => match e {
    ...
    UntypedASTMain::LetRecExp(id, para, exp1, exp2) => {
      let newenv = environment::extend(
        id.to_string(),
        Exval::RecProcV(id, para, *exp1, env.clone()),
        env,
      );
    ...
    UntypedASTMain::Apply(exp1, exp2) => {
      ...
      match funval.clone() {
        Exval::ProcV(id, body, env1) => {
          let env2 = environment::extend(id, arg, env1);
          eval_exp(env2, body)
        }
        Exval::RecProcV(id, para, body, env1) => {
          let env2 = environment::extend(para, funval, environment::extend(id, arg, env1));
          eval_exp(env2, body)
        }
